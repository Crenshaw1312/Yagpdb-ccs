...
`ttt stats [mention]` to get a users stats, ping no one to geet your own
`ttt reset` completely reset you stats
`ttt marker <1/2> <emoji>` set your marker for when youre player1 or player 2 to something besides X and O
*/}}
...
{{ define "adduser" }}
	{{ if not (dbGet .User.ID "tictactoe").Value }}
		{{ dbSet .User.ID "tictactoe" (sdict "Marker1" .Marker1 "Marker2" .Marker2 "Won" "0" "Tied" "0" "Lost" "0") }}
	{{ end }}
{{ end }}
{{ template "adduser" (sdict "User" .User "Marker1" $player1Marker "Marker2" $player2Marker) }}
...
{{ if eq $cmd "stats" }}
	...
	{{ template "adduser" (sdict "User" $member.User) }}
...
{{ else if eq $cmd "play" }}
	...
	{{ template "adduser" (sdict "User" $member.User "Marker1" $player1Marker "Marker2" $player2Marker) }}
...
{{ else }}
	{{ sendMessage nil (cembed
		...
		"Description" (joinStr "\n\n"
			...
			"`ttt reset` Reset all of your stats, markers and play scores."
		)
		"Footer" (sdict "icon_url" (.User.AvatarURL "256") "text" (print "Requested by " .User.String))
		"Color" 0x4B0082
