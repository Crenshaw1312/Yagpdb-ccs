{{/*Made by Crenshaw#7860,
	Regex: ^-pic(ture)?(\s(random|id(\s)?(\d){0,3}))?(\s)?((\d){1,3}x(\d){1,3})?$
	I hope you enjoy, I plan on updating this ocassionally*/}}
	 
	{{$base:="https://picsum.photos/"}}
	 
	{{if reFind `((\d){1,3}x(\d){1,3})` .Cmd}}
		 {{$wh:=reReplace `^-pic(ture)?(\s(random|id(\s)?(\d){0,3}))?(\s)?` .Cmd ""}}
		 {{$wh:=split $wh "x"}}
		 {{dbSet 0 "width" (index $wh 0)}}
		 {{dbSet 0 "height" (index $wh 1)}}
	{{else}}
		 {{dbSet 0 "width" "250"}}
		 {{dbSet 0 "height" "250"}}
	{{end}}
	...
	{{if reFind "id" .Cmd}}
		 ...
		 {{if eq $id ""}}
			  {{$id:= randInt 0 999}}
			  {{$image:=(joinStr "" $base "id/" $id "/" (dbGet 0 "width").Value "/" (dbGet 0 "height").Value)}}
			  {{sendMessage nil (cembed 
			  "title" (toString $id)
			  "image" (sdict "url" $image)
			  "color" 123
			  "timestamp" currentTime
			  "footer" (sdict "text" (joinStr "" "From " .User.String) "icon_url" (.User.AvatarURL "256")))}}
		 {{else}}
			  {{$image:=(joinStr "" $base "id/" $id "/" (dbGet 0 "width").Value "/" (dbGet 0 "height").Value)}}
			  {{sendMessage nil (cembed 
			  "title" (toString $id)
	...
	{{else if reFind "random" .Cmd}}
		 {{$image:=(joinStr "" $base (dbGet 0 "width").Value "/" (dbGet 0 "height").Value "?random=" (randInt 0 1000000000))}}
			  {{sendMessage nil (cembed 
			  "title" (title "Random")
			  "image" (sdict "url" $image)
			  "color" 123
